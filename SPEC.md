...
banking-common           ← shared domain objects, exception hierarchy, utilities
      ↑
      ├── banking-notification  ← event-driven email/SMS/push notification logging
      ...
      └── banking-fraud         ← rule-based fraud detection (2 MCP tools)
           ↑
banking-ai-gateway        ← Spring Boot entry point, ChatClient, MCP server, REST API (port 8080)
...
- No MCP tools; called internally by domain services
...
| Audit Log | `AuditLogAspect` (SLF4J) | Log aggregator in production |
...
**Excluded from coverage:** `**/domain/**`, `**/dto/**`, `**/config/**`, `**/exception/**`
...
| `SPRING_PROFILES_ACTIVE` | No | `default` | Set to `dev` to load demo data |
...
| Replace notification provider | Swap `NotificationService` implementation; all callers use Spring DI |
